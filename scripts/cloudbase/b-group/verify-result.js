...
function verifySnapshot(snapshot, plan) {
  ...
  assert(new Set(snapshot.collections.map((collection) => collection.collectionName)).size === plan.allowedCollections.length, 'Snapshot collections must not contain duplicates');
