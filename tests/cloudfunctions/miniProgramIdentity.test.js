...
function createMockDb(users = [], options = {}) {
  const state = { users: users.map((user) => ({ ...user })), audits: [], transactionCalls: 0 };
  ...
  const readCollection = (collection, transaction) => ({
    ...
    doc(id) {
      return {
        async get() {
          return { data: clone(state.users.find((user) => user._id === id)) };
        },
        async update({ data }) {
...
test('15. 停用身份档案返回 ACCOUNT_DISABLED', async () => {
...
test('16. 已被其他微信绑定的目标返回 ACCOUNT_ALREADY_BOUND', async () => {
...
test('17. 当前微信已有合法绑定返回 ALREADY_BOUND', async () => {
  const user = baseUser({ wxIdentityKey: 'openid:trusted-openid', wxOpenId: 'trusted-openid', bindStatus: 'bound' });
...
test('18. 正常 student 绑定在同一事务写入用户与审计', async () => {
...
test('20. version 并发冲突返回 CONFLICT', async () => {
...
test('21. wxIdentityKey UNIQUE 冲突返回 WECHAT_ALREADY_BOUND', async () => {
...
test('22. OPENID 只取服务端上下文，忽略 event.openid', async () => {
  const { handler, state } = makeBinding([baseUser()], trustedContext);
...
test('23. 审计 payload 不含 OPENID 等敏感字段', async () => {
  const { handler, state } = makeBinding([baseUser()], trustedContext);
...
test('24. 审计失败使绑定事务返回 INTERNAL_ERROR', async () => {
